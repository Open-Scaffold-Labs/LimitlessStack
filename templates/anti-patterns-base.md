...
**When verify_failed is non-zero for a NotebookLM file source — the fix is delete + re-add, NOT re-running the refresh script.** Re-running refresh (even with `--force`) will report success again and still do nothing — it's a structural no-op for file sources. The correct procedure every time `verify_failed > 0`:
1. `notebooklm source delete-by-title "<exact filename>"` → confirm with `y`
2. `notebooklm source add "<absolute path to file>"`
3. `notebooklm source wait <new_source_id>`
4. `notebooklm ask "..."` — query for a specific fact from the new content to confirm it landed
Do NOT try --force. Do NOT re-run the wiki refresh script. Delete and re-add is the only thing that works for file sources. (Added 2026-04-29 after discovering this in a The Match session.)

**Even delete+re-add can leave ghost duplicates — API eventual consistency.** Discovered later the same day (2026-04-29): `cmd_replace`'s "delete + verify gone + add new" flow assumed `notebooklm source list` would reflect the deletion immediately. It doesn't always. NotebookLM's listing endpoint has a ~1-2s window where a just-deleted source can appear gone in the list, then *come back* (or never have been deleted server-side at all). The post-delete check passes, `cmd_add` runs, and now there are TWO copies of the file in the notebook — one ghost-old and one fresh. Symptom: `notebooklm-wiki-refresh.py` reports `refreshed: N verify_failed: 0 upload_failed: 0` while the next preflight's dedupe sweep flags new dupes that didn't exist before the refresh. Observed twice in one session: `cdaa7a43`/claude-anti-patterns.md and `ab4b7ccb`/CLAUDE.md, with `ab4b7ccb`/claude-anti-patterns.md getting refreshed cleanly in the same run — so it's an intermittent race, not deterministic. **Corrective (deployed 2026-04-29)**: Added `_claim_unique_title(title, keep_sid)` helper to `notebooklm-wiki-refresh.py`. After every successful upload (cmd_replace AND new-file branches in sync_route + sync_reminder), the helper lists sources, finds any with the same title as the file just uploaded, and deletes all but the keep_sid. Plus a 2-second sleep before `cmd_replace`'s post-delete check to reduce false "looks gone" reports. Triple-defended now: (1) cmd_delete's pre/post check, (2) cmd_replace's abort-if-still-present, (3) post-add claim-by-title sweep. The structural rule generalizes: **for any "delete then add" pattern against an eventually-consistent API, the post-add claim sweep is required because no amount of post-delete polling can prove the delete propagated server-side.**

### 13. Silently freezing mid-task when a tool call stalls — and not pulling back

**What happens**: During a large multi-step write (e.g. appending several chunks to a file via desktop-commander), a tool call hangs — it neither completes nor times out cleanly. I wait silently for the response with no output, no warning, no indication to the user that anything is wrong. From the user's side the screen freezes for 10–20 minutes with zero progress. The stall only breaks when the user sends a new message, which gives me a new turn and forces the tool call to resolve — at which point I resume immediately as if nothing happened.

**Why it happens**: Each tool call is blocking — I can't act until the result comes back. Desktop-commander's `write_file` / `append` calls make a round-trip to the Mac filesystem and occasionally stall at the MCP layer without surfacing a timeout error I can catch. I have no built-in mechanism to detect "this has been waiting too long" and no way to cancel the pending call. So I just... wait. Indefinitely.

**Corrective**: Two rules. (1) **Before any large multi-chunk write, warn the user**: "This is going to take several tool calls — if I go quiet for more than 30 seconds, just send any message and it'll unblock me." Don't let them watch a frozen screen wondering if something broke. (2) **If a tool call takes more than one exchange without returning**, treat it as a stall signal on the next available turn — pull back, try a different approach (smaller chunk, different tool, splitting the work differently), and tell the user what happened. Don't retry the exact same stalling call pattern hoping it resolves on its own.

**Specific trigger**: Any task requiring 3+ sequential `write_file` / `append` calls to desktop-commander on a large file. Warn upfront. If stall is detected mid-task, report it rather than silently waiting.

### 14. Editing the deployed copy without back-porting to the canonical source — drifting LimitlessStack and Hub vault

**What happens**: A bug surfaces in the Hub vault's `tools/notebooklm-wiki-refresh.py` (or any other Hub-side script). I fix it directly in `/Users/matthewlavin/Claude code antigravity/obsidian /tools/`, test it, commit + push to the Hub vault repo, and consider the work done. But the canonical source for these scripts lives in `/Users/matthewlavin/LimitlessStack/tools/` — the same scripts that `install.sh` deploys to every new project. I never back-port the fix. Three days later, the Hub vault has 174 lines of fixes that LimitlessStack doesn't. Any new project installed via `install.sh` gets the OLD buggy version. Observed 2026-04-29: today's session fixed `cmd_replace`'s ghost-dupe hole, added `IGNORED_NOTEBOOKS`, added `--check-coverage`, added the routing-coverage contract — all in the Hub vault only. LimitlessStack still had the pre-fix versions, meaning new projects would have shipped with the bugs.

**Why it happens**: The Hub vault is what I'm "in" during a session — it has my CLAUDE.md, my wiki, my `tools/`. The mental model is "I'm working in the Hub project, I fix things in the Hub project." The fact that those `tools/` scripts are *deployed copies of canonical files maintained elsewhere* doesn't surface unless I'm explicitly looking at the LimitlessStack repo. The canonical-vs-deployed relationship is invisible from inside any one vault. The same pattern can drift `~/.claude/skills/` from `LimitlessStack/skills/` — every install of a new skill anywhere on the filesystem can drift the canonical source if not synced back.

**Corrective (deployed 2026-04-29 PM)**: Mechanical enforcement via `tools/limitless-preflight.sh` — new "Limitless Stack canonical sync" check diffs `$VAULT/tools/*` and `~/.claude/skills/*` against `$LIMITLESS_STACK_HOME/tools/*` and `$LIMITLESS_STACK_HOME/skills/*` (default `/Users/matthewlavin/LimitlessStack`). Any drift fires a ⚠ on the next session's Roll Call with the exact `cp` command to resolve. The check runs on every preflight, so it's caught within one session of drift. **Operational rule**: every time you edit `tools/<anything>` or a skill in `~/.claude/skills/` or `LimitlessStack/skills/`, before commit + push, run the preflight; if the sync check warns, fix the drift FIRST, THEN commit. Both repos (`Open-Scaffold-Labs/LimitlessStack` and `Open-Scaffold-Labs/openscaffold-wiki`) get committed in the same logical change.

**Specific trigger**: Any edit to `<vault>/tools/*.{sh,py}` or to a skill SKILL.md anywhere. The sync contract applies in BOTH directions — Hub→LimitlessStack and LimitlessStack→Hub.

**The pair to remember**: Hub vault = `/Users/matthewlavin/Claude code antigravity/obsidian`; canonical = `/Users/matthewlavin/LimitlessStack`. They are not optional partners. The "Limitless Stack continues to learn over time" property of the system depends entirely on them staying in sync.
