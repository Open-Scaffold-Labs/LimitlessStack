...
### 13. Mechanical checks without semantic checks

**What happens**: The Limitless Stack preflight grew a comprehensive set of mechanical checks — files committed, repos pushed, timestamps consistent, tools byte-equal with canonical, NotebookLM dedupe sweep, sync state file fresh. By 2026-05-07 it had ~12 such checks. None of them looked at the *content* of the documents being checked. The result: a session discovered that a project's CLAUDE.md "Feature status" table had been claiming features as "🔲 Next" for the entire week after they shipped, that `wiki/index.md` was missing pages, that `wiki/overview.md` had been a `YYYY-MM-DD` template placeholder for 10 days, and that `HIGH-PRIORITY-TODO.md` (a security-critical secret rotation) had sat 3 days past its stated deadline. Every preflight in those 10 days reported READY/WARN with no mention of any of it.

**Why it happens**: Mechanical checks are deterministic and easy to write — compare two file hashes, check a timestamp, count pages. They produce zero false positives when correct. Semantic checks require the script to interpret content — *is* this status claim still true? *did* we miss this deadline? Those are heuristic by nature and risk false positives. So the early Limitless Stack got built with all the mechanical safeguards first and the semantic ones deferred. The deferral became permanent invisibly: every session prioritized the user's stated need (ship the feature) over updating meta-docs, and without a mechanical step, the meta-doc maintenance got skipped every session. The trust anchor (`CLAUDE.md`) drifted from reality while every layer below it (sync, refresh, dedupe) stayed perfectly consistent.

**Corrective**: Mechanical checks alone don't make a trust anchor trustworthy — they make the storage layer reliable while the content drifts. For any system where humans rely on a document as ground truth (`CLAUDE.md`, an index, a TODO list), the preflight must include semantic checks: index completeness, template-placeholder detection, overdue-deadline scans. False positives are the cost of admission; the alternative is silent staleness, which is the bug we're trying to prevent. Added 2026-05-07: three new semantic checks in `tools/limitless-preflight.sh` (index completeness against `wiki/*.md`, `YYYY-MM-DD` frontmatter detection, TODO overdue scan) plus a new procedural step in both CLAUDE.mds — *re-read the trust anchor end-to-end before commit* — to catch the cases the heuristics miss.

**Specific trigger**: Whenever you find yourself building the Nth mechanical check on the Limitless Stack ("verify the file exists / matches the canonical / has fresh timestamps"), pause and ask: is the failure mode I just discovered actually about file consistency, or about whether the file's *content* is still true? If it's the latter, the mechanical check won't catch it — build the semantic check too. Same trigger for "I noticed the CLAUDE.md says X but reality is Y" — that's a semantic-check candidate, not a documentation-fix candidate.
